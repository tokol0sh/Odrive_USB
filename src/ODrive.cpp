...
public:
	...
	int endpoint_request(int endpoint_id, serial_buffer& received_payload, std::vector<uint8_t> payload, int ack, int length) {
		...
		// Send the packet
		send_to(handle, create_odrive_packet(seq_no, endpoint_id, (short)length, (std::vector<uint8_t>)payload), &sent_bytes);


		// Immediatly wait for response from Odrive and check if ack (if we asked for one)
		receive_from(handle, receive_bytes, max_bytes_to_receive, &received_bytes, timeout);
...
private:
	...
	void send_to(libusb_device_handle* handle, std::vector<uint8_t>& packet, int* sent_bytes) {
	...
	void receive_from(libusb_device_handle* handle, unsigned char* packet, int max_bytes_to_receive, int* received_bytes, int timeout = 0) {
		libusb_bulk_transfer(handle,
